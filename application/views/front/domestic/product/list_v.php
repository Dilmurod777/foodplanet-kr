...
<div class="container">
		<div class="sub-container">
			<div class="data-list domestic product"><!-- 국내데이터 class="domestic" 제품 class="product" -->
				<div class="sub-visual type1"><!-- D:20230611 class="type1" 추가 -->
...
<form id="frmSearch">
	<input type="hidden"  name="biz_no" value="<?php echo !empty($req['biz_no']) ? $req['biz_no'] : ''; ?>" />
	<input type="hidden" name="prod_type" value="<?php echo !empty($req['prod_type']) ? $req['prod_type'] : ''; ?>" />
	...
	$.ajax({
			...
			success: function(data) {
				...
				for(var i = 0;  i < data.list.length; i++) {
					...
					str += '<li>'
						+	'	<a href="#" onclick="javascript:goDetail(\'' + data.list[i].seq + '\'); return false;">'
						...
						+	'		<span class="cate">' + (tags.length > 0 && tags[0] !== '' ? '#' + tags.join('#') : '') + '</span>'
...
function goDetail(seq) {
	...
	$('#frmSearch').attr('action', '/domestic/product/detail');
