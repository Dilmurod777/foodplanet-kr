        ...
        <div id="page-wrapper" class="gray-bg">
        	...
        	<div class="wrapper wrapper-content animated fadeInRight">
                            	...
                            	<table class="footable table table-stripped">
									...
									?>
                                                <tr>
													...
													<td class="text-center"><img src="<?php echo $row['prod_img']; ?>" style="max-width:100px; max-height:100px;"></td>
                ...
                <div class="form-group text-center">
                    <button type="button" class="btn btn-w-m btn-success" onclick="javascript:fnShowRegister(); return false;">제품추가</button>
                </div>
...
function fnChangeOrder(code, no, dir)
{
	$.ajax({
	   	type:'POST',
	   	url:'/api/admin/manager/product10/change_order',
...
function fnDelete(code) {
	...
	$.ajax({
	   	type:'POST',
	   	url:'/api/admin/manager/product10/delete',
		data : { recommend_seq : code },
		dataType:"json",
       	success:function(data){
...
function fnRegister(seq, type) {
	$.ajax({
	   	type:'POST',
	   	url:'/api/admin/manager/product10/register',
		data : {product_seq : seq, product_type : type},
		dataType:"json",
       	success:function(data){
       		if(typeof(data.result) == 'login') {
                alert(data.msg);
...
function goPage(page) {
	...
	$.ajax({
       	type:'POST',
    	url:'/api/admin/domestic/product/list',
       		...
       		if(product.length <= 0) {
       			...
       			for(var i = 0; i < product.length; i++) {
	       			table += '<tr>'
       					...
       					+ ' <td class="text-center"><img src="' + product[i].prod_img + '" style="max-width:100px; max-height:100px;"></td>'
       					+ ' <td>' + product[i].company_name + '</td>'
       					+ '	<td>' + product[i].product_name + '</td>'
       					+ ' <td class="text-center"><button type="button" class="btn btn-w-m btn-primary" onclick="javascript:fnRegister(\'' + product[i].seq + '\', \'' + product[i].product_type2 + '\'); return false;">선택</button></td>'
       					+ '</tr>';
       			}
