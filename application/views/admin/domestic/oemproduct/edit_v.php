									...
									<div class="form-group  row">
										...
										<?php
											foreach($company as $row) {
												echo '<div class="col-lg-3 check_wrap">';
												echo '	<input type="radio" id="company_' .  $row['sub_code'] . '" name="main_oem" value="' . $row['code_name'] . '" ' . ($row['code_name'] === $info['main_oem'] ? 'checked' : '') . ' >';
...
function fnSave()
{
	...
	$.ajax({
		url: "/api/admin/domestic/oemproduct/edit",
		type: "POST",
		data: data,
...
function fnDelete() {
	...
	$.ajax({
		url: "/api/admin/domestic/oemproduct/delete",
		type: "POST",
