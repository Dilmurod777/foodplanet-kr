...
Class Channel extends FP_AdminController {
	...
	public function list() {
		...
		$total_rows = $this->overseas_m->channel_list_cnt($req);
		$num = $total_rows - $offset;
		
		$pagination = $this->pagination(5, $total_rows, $offset, PERPAGE);

        $list = $this->overseas_m->channel_list($req, $offset, PERPAGE)->result_array();
		...
		$this->load->view('admin/overseas/channel/list_v');
	...
	public function register() {
		...
		$this->load->view('admin/common/include/header_v', $this->data);
		$this->load->view('admin/overseas/channel/register_v');
		$this->load->view('admin/common/include/footer_v');
	}
	...
	public function edit() {
        $req = $this->input->post();
        if(empty($req['seq'])) {
            header('Location: /admin/overseas/channel/list');
            exit;
        }
        $info = $this->overseas_m->channel_info($req['seq'])->row_array();
        if(empty($info)) {
            header('Location: /admin/overseas/channel/list');
            exit;
        }
		...
		$this->data['product'] = $this->overseas_m->product_list_all()->result_array();
		$this->load->view('admin/common/include/header_v', $this->data);
		$this->load->view('admin/overseas/channel/edit_v');
		$this->load->view('admin/common/include/footer_v');
	}
}