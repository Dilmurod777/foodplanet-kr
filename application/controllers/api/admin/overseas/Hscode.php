...
Class Hscode extends FP_ApiController {
	...
	public function register()
	{
		...
		if($result['result'] == 'succ') {
			...
			$res = $this->overseas_m->insert_hscode($req);

			if($res) {
				$result['result'] = 'succ';
				$result['msg'] = '등록되었습니다.';
				$this->log_m->insert_log('tb_overseas_hscode', 'C', $req, $this->data['admin']['admin_id']);
			}
			else {
				$result['result'] = 'fail';
	...
	public function edit()
	{
		...
		if($result['result'] == 'succ') {
			...
			$res = $this->overseas_m->update_hscode($req);

			if($res) {
				$result['result'] = 'succ';
				$result['msg'] = '수정되었습니다.';
				$this->log_m->insert_log('tb_overseas_hscode', 'U', $req, $this->data['admin']['admin_id']);
			}
			else {
				$result['result'] = 'fail';
	...
	public function delete()
	{
		...
		if(empty($req['seq'])) {
			...
			$req['admin_id'] = $this->data['admin']['admin_id'];
			$res = $this->overseas_m->delete_hscode($req);
			if($res) {
				$result['result'] = 'succ';
				$result['msg'] = '삭제되었습니다.';
				$this->log_m->insert_log('tb_overseas_hscode', 'D', $req, $this->data['admin']['admin_id']);
			}
			else {
				$result['result'] = 'fail';
