...
Class Patent extends FP_ApiController {
        ...
        if(empty($req['chk_biz_no']) || $req['chk_biz_no'] === 'n') {
            ...
            if($result['result'] == 'succ') {
                ...
                if(!empty($_FILES) && !empty($_FILES['img_url_attach']) && !empty($_FILES['img_url_attach']['tmp_name']) && !empty($_FILES['img_url_attach']['name'])) {
                    ...
                    if($result['result'] === 'succ') {
                        $req['patent_img'] = $result['data'];
                    }
                }
                $req['admin_id'] = $this->data['admin']['admin_id'];

                $this->domestic_m->insert_patent($req);
                $result['msg'] = '등록하였습니다.';
                $this->log_m->insert_log('tb_domestic_patent', 'C', $req, $this->data['admin']['admin_id']);
    ...
    public function edit()
    {
        ...
        if($result['result'] == 'succ') {
            ...
            $this->domestic_m->update_patent($req);
            $result['msg'] = '수정하였습니다.';
            $this->log_m->insert_log('tb_domestic_patent', 'U', $req, $this->data['admin']['admin_id']);
        }
    ...
    public function delete()
    {
        ...
        if(empty($req['seq'])) {
            $result['result'] = 'fail';
            $result['msg'] = '삭제할 특허을 선택해 주세요.';
        }
        else {
            $req['admin_id'] = $this->data['admin']['admin_id'];

            $this->domestic_m->delete_patent($req);

            $result['result'] = 'succ';
            $result['msg'] = '삭제되었습니다.';
            $this->log_m->insert_log('tb_domestic_patent', 'D', $req, $this->data['admin']['admin_id']);
        }
    ...
    public function chk_require($req) {
        ...
        else if(empty($req['patent_name'])) {
            $result['result'] = 'fail';
            $result['msg'] = '특허명을 입력해 주세요.';
        }
        else if(empty($req['patent_name_eng'])) {
            $result['result'] = 'fail';
            $result['msg'] = '특허 영문명을 입력해 주세요.';
        }
        return $result;
    }
}