...
Class Report extends FP_ApiController {
	...
	public function register()
	{
		...
		if(empty(trim($req['title']))) {
			...
			$res = $this->report_m->insert_report($req);
	...
	public function update()
	{
		...
		$result = array();
		if(empty($req['report_seq'])) {
			$result['result'] = 'fail';
			$result['msg'] = '수정할 리포트를 선택해 주세요.';
		}
			...
			$res = $this->report_m->update_report($req);
	...
	public function delete()
	{
		...
		if(empty($req['report_seq'])) {
			...
			$req['member_id'] = $this->data['admin']['admin_id'];
			$res = $this->report_m->delete_report($req);
			if($res) {
				$result['result'] = 'succ';
				$result['msg'] = '삭제되었습니다.';
			}
